...
describe("Task Contract", function () {
  ...
  beforeEach(async function () {
    ...
    for (let i = 0; i < NUM_TOTAL_TASKS; i++) {
      let task = {
        askingAmount: "Amount number:- " + i,
        spread: "spread number:- " + i,
        isDeleted: false,
      };

      await taskContract.addBids(
        task.askingAmount,
        task.spread,
        task.isDeleted
      );
  ...
  describe("Add Task", function () {
    it("should emit AddBids event", async function () {
      let task = {
        askingAmount: "New Task",
        spread: "80",
        isDeleted: false,
      };
